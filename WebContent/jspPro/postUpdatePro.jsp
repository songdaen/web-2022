...
<%
  ...
  Connection con = ds.getConnection();                        //커넥션 풀로부터 커넥션 객체를 얻어냄 
	...
	//3. SQL문 
	String sql = "update post set title=?, contents=? where postno=?";  // 특정 ID가 속한 행의 name과 pwd를 수정하는 쿼리문 
	
	//4. SQL 실헹 
	PreparedStatement pstmt = con.prepareStatement(sql);  // PreparedStatement 객체 선언
	pstmt.setString(1, request.getParameter("title"));   // pstmt에 ID 값 셋팅 
	pstmt.setString(2, request.getParameter("contents"));  // pstmt에 패스워드 값 셋팅 
